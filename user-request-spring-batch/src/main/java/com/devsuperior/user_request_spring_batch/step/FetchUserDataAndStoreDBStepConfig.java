package com.devsuperior.user_request_spring_batch.step;
...
public class FetchUserDataAndStoreDBStepConfig {
	...
	public Step fetchUserDataAndStoreDBStep(ItemReader<UserDTO> fetchUserDataReader,
		...
		return new StepBuilder("fetchUserDataAndStoreDBStep", jobRepository)
				.<UserDTO, User>chunk(chunkSize, transactionManager).reader(fetchUserDataReader)
				.processor(selectFieldsUserDataProcessor).writer(insertUserDataDBWriter).build();
	}
